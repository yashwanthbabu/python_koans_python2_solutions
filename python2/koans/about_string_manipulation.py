...
class AboutStringManipulation(Koan):
    ...
    def test_use_format_to_interpolate_variables(self):
        ...
        self.assertEqual(string.format(value1,value2), string)
    ...
    def test_any_python_expression_may_be_interpolated(self):
        ...
        string = "The square root of 5 is {0:.{1}f}".format(math.sqrt(5), \
            decimal_places)
        self.assertEqual(string.format(math.sqrt(5)), string)
    ...
    def test_raw_strings_do_not_interpret_escape_characters(self):
        ...
        self.assertNotEqual('\n', string)
        self.assertEqual('\\n', string)
        self.assertEqual(string.count('')-1, len(string))

        # Useful in regular expressions, file paths, URLs, etc.

    def test_strings_can_be_joined(self):
        words = ["Now", "is", "the", "time"]
        self.assertEqual(words[0]+' '+words[1]+' '+words[2]+' '+words[3], ' '.join(words))
